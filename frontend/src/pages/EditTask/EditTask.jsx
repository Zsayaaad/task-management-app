...
const EditTask = () => {
  ...
  const isCreator = (task?.creatorId || task?.creator?.id) === user?.id;
  const isAssignee = (task?.assigneeId || task?.assignee?.id) === user?.id;
  const isOnlyAssignee = !isAdmin && !isCreator && isAssignee;
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-surface-container border border-border rounded-xl p-6 shadow-lg">
        <Form method="post" className="space-y-5" noValidate>
          {isOnlyAssignee ? (
            /* If user is only assignee: render status field only */
            <div>
              <label
                htmlFor="status"
